 ...
 *
 * This function relies on the input being in _roughly_ reading order, more so
 * at the low (word, line) level. When the input is not in reading order, the
 * output may be divided into more lines / columns than expected.
 *
 * @param {DOMRect[]} chars
 * @param {string} text
 * @return {ColumnBox[]}
    ...
    for (let column of columns) {
      ...
      for (let line of column.lines) {
        ...
        for (let word of line.words) {
          ...
          // Scale text using a transform to account for differences between the
          // text layer font and original document font. The transform affects
          // the visual size, used by text selection, but not CSS layout.
