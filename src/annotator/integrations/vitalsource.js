...
export function vitalSourceFrameRole(window_ = window) {
  // Is this the container frame?
  if (findBookElement(window_.document)) {
    return 'container';
  }

  // Is this the chapter content frame?
 ...
 * configure and load the client into the book content frame.
 *
...
export class VitalSourceContainerIntegration {
  constructor(annotator) {
    ...
    if (!bookElement) {
      throw new Error('<mosaic-book> container not found');
    }
    ...
    const mo = new MutationObserver(injectClientIntoContentFrame);
