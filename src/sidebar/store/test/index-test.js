...
describe('sidebar.store.index', function () {
  var store;
  var fakeRootScope;

  beforeEach(function () {
    fakeRootScope = {$applyAsync: sinon.stub()};
    store = createStore(fakeRootScope, {});
  });
  ...
  describe('initialization', function () {
    ...
    it('sets the selection when settings.annotations is set', function () {
      store = createStore(fakeRootScope, {annotations: 'testid'});
  ...
  describe('#addAnnotations()', function () {
    ...
    it('preserves anchoring status of updated annotations', function () {
      var annot = defaultAnnotation();
      store.addAnnotations([annot]);
      store.updateAnchorStatus(annot.id, null, false /* not an orphan */);
    ...
    it('does not set the timeout flag on annotations that do anchor within a time limit', function () {
      var annot = defaultAnnotation();
      store.addAnnotations([annot]);
      store.updateAnchorStatus(annot.id, 'atag', false);
  ...
  describe('#updatingAnchorStatus', function () {
    it("updates the annotation's tag", function () {
      var annot = defaultAnnotation();
      store.addAnnotations([annot]);
      store.updateAnchorStatus(annot.id, 'atag', true);
      assert.equal(store.getState().annotations[0].$tag, 'atag');
    });
    ...
    it('updates annotations by tag', function () {
      store.addAnnotations(fixtures.newPair);
      store.updateAnchorStatus(null, 't2', true);
