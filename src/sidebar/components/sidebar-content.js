...
function SidebarContent({
  ...
  const hasDirectLinkedAnnotationError = useStore(store => {
    if (annotationsLoading || !linkedAnnotationId) {
      return false;
    }
    return !store.annotationExists(linkedAnnotationId);
  });
  ...
  // Connect to the streamer when the sidebar has opeend or if user is logged in
  useEffect(() => {
