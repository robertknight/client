...
function SidebarContentController(
  $scope, analytics, store, annotationMapper, features, frameSync,
  ...
  $scope.$on(events.ANNOTATIONS_SYNCED, function (event, tags) {
    ...
    var separateOrphans = tabs.shouldSeparateOrphans(store.getState());
    store.selectTab(tabs.tabForAnnotation(selectedAnnot, separateOrphans));
  });
