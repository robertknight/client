...
describe('sidebar.api-client', function () {
  ...
  before(function () {
    angular.module('h', [])
      .service('apiClient', proxyquire('../api-client', util.noCallThru({
        angular: angular,
        './retry-util': {
  ...
  it('saves a new annotation', function (done) {
    apiClient.annotation.create({}, {}).then(function (saved) {
  ...
  it('updates an annotation', function (done) {
    apiClient.annotation.update({id: 'an-id'}, {text: 'updated'}).then(function () {
      done();
    });
  ...
  it('deletes an annotation', function (done) {
    apiClient.annotation.delete({id: 'an-id'}, {}).then(function () {
      done();
    });
  ...
  it('flags an annotation', function (done) {
    apiClient.annotation.flag({id: 'an-id'}).then(function () {
      done();
    });
  ...
  it('hides an annotation', function (done) {
    apiClient.annotation.hide({id: 'an-id'}).then(function () {
      done();
    });
  ...
  it('unhides an annotation', function (done) {
    apiClient.annotation.unhide({id: 'an-id'}).then(function () {
      done();
    });
  ...
  describe('#group.member.delete', () => {
    it('removes current user from a group', (done) => {
      apiClient.group.member.delete({pubid: 'an-id', user: 'me'}).then(function () {
        done();
      });
  ...
  it('removes internal properties before sending data to the server', function (done) {
    ...
    apiClient.annotation.create({}, annotation).then(function () {
      done();
    });
  ...
  it('encodes semicolons in query parameters', function (done) {
    apiClient.search({'uri': 'http://example.com/?foo=bar;baz=qux'}).then(function () {
      done();
    });
  ...
  it("fetches the user's profile", function (done) {
    ...
    apiClient.profile.read({authority: 'publisher.org'}).then(function (profile_) {
  ...
  it("updates a user's profile", function (done) {
    apiClient.profile.update({}, {preferences: {}}).then(function () {
      done();
    });
  ...
  context('when an API calls fail', function () {
    util.unroll('rejects the call with an Error', function (done, testCase) {
      apiClient.profile.update({}, {preferences: {}}).catch(function (err) {
